...
function userOptions(){
	document.getElementById('properties').style.display="block";
	document.getElementById('properties').innerHTML="<form class='forma'>"+
	"<label for='newPass'>New password:</label><br/>"+
	"<input id='newPass' type='password' name='newPass'><br/>"+
	"<label for='oldPass'>Old password:</label><br/>"+
	"<input id='oldPass' type='password' name='oldPass'><br/>"+
	"<input type='button' name='changePass' id='changePass' value='Change password' onClick='changeUser(this);'><br/>"+
	"<p id='passError' class='error'></p>"+
	"<p id='passSuccess'></p>"+
	"<label for='email'>Email:</label><br/>"+
	"<input id='email' type='textbox' name='email'><br/>"+
	"<input type='button' name='changeEmail' id='changeEmail' value='Change email' onClick='changeUser(this);'><br/>"+
	"<p id='emailError' class='error'></p>"+
	"<p id='emailSuccess'></p>"+
	"<input type='button' id='login' value='Log out' onClick='logout();'><input type='button' value='Cancel' onClick='sakriProperties();'>"+
	"</form>";
}

function changeUser(param){
	if(param.id=="changePass"){
		var greske="";
		var regPass=/^[\w\n\S]{4,20}$/;
		...
		if(regPass.test(newPass)){
			...
			document.getElementById('passError').innerHTML="Must be longer than 4 characters";
			greske="error";
		}
		if (greske == "") {
	    	var vars = "newPass="+newPass+"&oldPass="+oldPass+"&option="+param.id;
			var hr=new XMLHttpRequest();
			hr.open("POST","useroptions.php",true);
			hr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			hr.onreadystatechange = function() {
			    if(hr.readyState == 4 && hr.status == 200) {
				    var return_data = hr.responseText;
					document.getElementById("passSuccess").innerHTML = return_data;
			    }
	    	}
			hr.send(vars);
	    }
		...
		if (greske == "") {
	    	var vars = "Mail="+Email+"&option="+param.id;
			var hr=new XMLHttpRequest();
			hr.open("POST","useroptions.php",true);
			hr.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			hr.onreadystatechange = function() {
			    if(hr.readyState == 4 && hr.status == 200) {
				    var return_data = hr.responseText;
					document.getElementById("emailSuccess").innerHTML = return_data;
			    }
	    	}
			hr.send(vars);
	    }
